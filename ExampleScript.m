...
if (0)
    matrix = zeros(size( Gyroscope, 1), 10);
    for i = 1:3
        matrix( :, i) = Gyroscope(:,i);
        matrix( :, i+3) = Accelerometer(:,i);
        matrix( :, i+6) = Magnetometer(:,i);
    end
    matrix( :, 10) = time;
    Savetotxt = save2txt( 'SampleData.txt', matrix );
end
...
AHRS = MadgwickAHRS('SamplePeriod', 1/256, 'Beta', 0.1);     %输入参量，采样频率256hz, Beta值选择0.1，还可以输入默认四元数Quaternion
% AHRS = MahonyAHRS('SamplePeriod', 1/256, 'Kp', 0.5);

quaternion = zeros(length(time), 4);
for t = 1:length(time)
    AHRS.Update(Gyroscope(t,:) * (pi/180), Accelerometer(t,:), Magnetometer(t,:));	% gyroscope units must be radians
    quaternion(t, :) = AHRS.Quaternion;                                                                     %记录每一次计算之后的结果
end
